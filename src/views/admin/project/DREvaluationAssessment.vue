<template>
  <validation-observer ref="drEvaluationAssessmentRules">
    <b-form @submit.prevent>
      ...
      <b-row>
        ...
        <b-col md="12">
          <b-form-group>
            <label>Berita Acara Sidang Evaluasi Design Recognition</label>
            <validation-provider
              #default="{ errors }"
              name="Berita Acara Sidang Evaluasi Design Recognition"
              rules="required"
            >
              <b-form-file
                v-model="files"
...
export default {
  ...
  setup() {
    ...
    const drEvaluationAssessmentDocument = ref({})
    ...
    store.dispatch('app-project/fetchProjectAttachmentsByFileType', { taskId: router.currentRoute.params.id, fileType: 'dr_evaluation_assessment' })
      .then(response => {
    ...
    return {
      projectData,
      drEvaluationAssessmentDocument,
  ...
  methods: {
    ...
    submitProject() {
      this.$refs.drEvaluationAssessmentRules.validate().then(success => {
        if (success) {
          ...
          this.$http.post('/engine-rest/new-building/dr_evaluation_assessment', request, config).then(res => {
